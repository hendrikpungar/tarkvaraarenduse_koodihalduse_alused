...
```{r}
Isd <- read_csv('./andmed/Islander_data.csv')
...
ggplot(Isd, aes(x = Drug, y = Diff)) +
  ...
  geom_point(position = position_jitter(width = 0.2), alpha = 0.1, size = 1) +
  theme_minimal()
```
...
p1 <- ggplot(Isd_refactored, aes(x = Drug_name, y = Diff, fill = Drug_name)) +
  geom_boxplot() +
  labs(x = "Ravim", y = "Mälu skoori erinevus") +
  theme_minimal() +
  theme(legend.position = "none")

p2 <- ggplot(Isd_refactored, aes(x = age, y = Diff, color = Drug_name)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Vanus", y = "Mälu skoori erinevus", color = "Ravim") +
  theme_minimal()

p3 <- ggplot(Isd_refactored, aes(x = Dosage, y = Diff, fill = Drug_name)) +
  geom_col(position = "dodge") +
  labs(x = "Doos", y = "Mälu skoori erinevus") +
  theme_minimal() +
  theme(legend.position = "none")

p4 <- ggplot(Isd_refactored, aes(x = Happy_Sad_group, y = Diff, fill = Happy_Sad_group)) +
  geom_violin() +
  labs(x = "Emotsiooni rühm", y = "Mälu skoori erinevus") +
  theme_minimal() +
  theme(legend.position = "none")

(p1 + p2) / (p3 + p4) +
  plot_annotation(
    title = "Ravimite mõju analüüs mälule",
    caption = "Andmeallikas: Islander andmestik"
  ) +
  plot_layout(guides = "collect")
```
