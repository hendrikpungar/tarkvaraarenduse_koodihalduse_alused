...
# 10 inimese toit
number_of_people <- 10
number_of_people * food_cost_per_person
number_of_people * drink_cost_per_person



total_party_cost <- function(n=10) { # n - osalejate arv
  venue_cost + (n*food_cost_per_person+n*drink_cost_per_person)+entertainment_cost
}
...
# 2. Kõigi klasside vanused ja tulemused
vanused <- c(20, 21, 19, 22, 20, 23, 24, 22, 25)
tulemused <- c(85, 78, 90, 88, 76, 95, 87, 80, 92)

# Loo maatriks 9 reaga ja 2 veeruga
opilaste_maatriks <- matrix(c(vanused, tulemused), nrow = 9, ncol = 2)
colnames(opilaste_maatriks) <- c("Vanus", "Tulemus")
rownames(opilaste_maatriks) <- c("Bio1", "Bio2", "Bio3", "Keemia1", "Keemia2", "Keemia3", "Füüsika1", "Füüsika2", "Füüsika3")

print("Õpilaste maatriks:")
print(opilaste_maatriks)

# 3. Kõik õpilased
nimed <- c("Anna", "Mike", "Luis", "Sam", "Ella", "Ravi", "Mia", "Jack", "Olivia")
klassid <- c("Bioloogia", "Bioloogia", "Bioloogia", "Keemia", "Keemia", "Keemia", "Füüsika", "Füüsika", "Füüsika")

opilased_df <- data.frame(
  nimi = nimed,
  vanus = vanused,
  tulemus = tulemused,
  klass = klassid
...
print("Anna andmed listina:")
print(anna_list)

# 5. Analüüs
# a)õpilased, kelle tulemus on üle 85
print("Õpilased tulemusega üle 85:")
head_opilased <- opilased_df[opilased_df$tulemus > 85, c("nimi", "klass")]
print(head_opilased)

# b) iga klassi keskmine tulemus maatriksist
print("\nKlasside keskmised tulemused:")
bio_keskmine <- mean(opilaste_maatriks[1:3, "Tulemus"])
keemia_keskmine <- mean(opilaste_maatriks[4:6, "Tulemus"])
fuusika_keskmine <- mean(opilaste_maatriks[7:9, "Tulemus"])

print(paste("Bioloogia keskmine:", round(bio_keskmine, 2)))
print(paste("Keemia keskmine:", round(keemia_keskmine, 2)))
print(paste("Füüsika keskmine:", round(fuusika_keskmine, 2)))
```
...
**VASTUS: read_tsv funktsioonil on 20 argumenti.**
...
```{r}
Isd = read_delim("./andmed/Islander_data.csv")

# Kuva andmestiku esimesed 10 rida
head(Isd, 10)

# Arvuta mälu skooride üldine keskmine enne ja pärast ravimi manustamist
mean(Isd$Mem_Score_Before, na.rm = TRUE)
mean(Isd$Mem_Score_After, na.rm = TRUE)
mean(Isd$Diff, na.rm = TRUE)

# 2. Ravimite analüüs
ravimite_analyys <- Isd %>%
  group_by(Drug) %>%
  summarize(
    keskmine_diff = mean(Diff, na.rm = TRUE),
    osalejate_arv = n()
  ) %>%
  arrange(desc(keskmine_diff))

cat("\n2. Ravimite analüüs:\n")
print(ravimite_analyys)
...
doosi_analyys <- Isd %>%
  ...
  arrange(Drug, desc(keskmine_diff))

cat("\n3. Doosi analüüs:\n")
print(doosi_analyys)

# 4. Vanuserühma analüüs
vanuseruhma_analyys <- Isd %>%
  mutate(
    age_group = case_when(
      age < 30 ~ "Noor",
  ...
  arrange(age_group, desc(keskmine_diff))

cat("\n4. Vanuserühma analüüs:\n")
print(vanuseruhma_analyys)

cat("\nJäreldus: Vaadates erinevate vanuserühmade tulemusi, võib märgata, et ravimi tõhusus võib erineda vanuserühmade lõikes.\n")

# 5. Õnnelik/Kurb rühma analüüs
emotsiooni_analyys <- Isd %>%
  group_by(`Happy_Sad_group`) %>%
  summarize(
    keskmine_enne = mean(Mem_Score_Before, na.rm = TRUE),
    keskmine_parast = mean(Mem_Score_After, na.rm = TRUE),
    keskmine_diff = mean(Diff, na.rm = TRUE),
    osalejate_arv = n(),
    .groups = 'drop'
  )
...
cat("\n5. Õnnelik/Kurb rühma analüüs:\n")
print(emotsiooni_analyys)

cat("\nJäreldus: Emotsiooni priming mõjutab mälu skoori. Võrdleme H (happy) ja S (sad) rühmade keskmist erinevust mälu skoorides.\n")
```
